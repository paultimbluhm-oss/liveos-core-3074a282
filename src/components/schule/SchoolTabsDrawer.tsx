...
export function SchoolTabsDrawer({ open, onOpenChange, context, course }: SchoolTabsDrawerProps) {
  ...
  const [courseSlots, setCourseSlots] = useState<Array<{
    id: string;
    day_of_week: number;
    period: number;
    room: string | null;
  ...
  const toggleEva = async (dayOfWeek: number, period: number, currentlyHasEva: boolean) => {
    if (!user) return;
    ...
    if (currentlyHasEva) {
      ...
      // First delete any existing entry at this slot
      await supabase
        .from('timetable_entries')
        .delete()
        .eq('user_id', user.id)
  ...
  // Check if a slot has EVA
  const slotHasEva = (dayOfWeek: number, period: number) => {
    return timetableEntries.some(
      e => e.day_of_week === dayOfWeek && e.period === period && e.teacher_short === 'FREI' && !e.course_id
    );
  };

  // Render course timetable slots
  const renderCourseTimetableSlots = () => {
    if (!course || courseSlots.length === 0) {
      return (
        <div className="py-8 text-center">
          <Clock className="w-8 h-8 mx-auto text-muted-foreground/30 mb-2" strokeWidth={1.5} />
          <p className="text-sm text-muted-foreground">Keine Stunden fuer diesen Kurs</p>
        </div>
      );
    }

    // Get unique primary slots - either is_double_lesson=true (start of double) or slots that are not part of a double lesson
    const nextPeriods: { [key: number]: number } = { 1: 2, 3: 4, 5: 6, 8: 9 };
    const prevPeriods: { [key: number]: number } = { 2: 1, 4: 3, 6: 5, 9: 8 };
    
    const uniqueSlots = courseSlots.filter(slot => {
      // If this is the start of a double lesson, include it
      if (slot.is_double_lesson) return true;
      
      // If this is NOT a double lesson, check if it's the second part of a double
      // by looking if there's a slot in the previous period that is a double lesson
      const prevPeriod = prevPeriods[slot.period];
      if (prevPeriod) {
        const prevSlot = courseSlots.find(s => 
          s.day_of_week === slot.day_of_week && s.period === prevPeriod && s.is_double_lesson
        );
        // If previous is a double lesson, skip this one (it's the second part)
        if (prevSlot) return false;
      }
      
      // It's a standalone single lesson
      return true;
    });
    
    return (
      <div className="space-y-2">
        <p className="text-xs text-muted-foreground mb-3">Stunden dieses Kurses</p>
        {uniqueSlots.map(slot => {
          const hasEva = slotHasEva(slot.day_of_week, slot.period);
          const nextPeriod = nextPeriods[slot.period];
          const hasEvaNext = nextPeriod ? slotHasEva(slot.day_of_week, nextPeriod) : false;
          const isDouble = slot.is_double_lesson && nextPeriod;
                  ...
                  <div className="flex items-center gap-3">
                    <div 
                      ...
                      style={{ 
                        ...
                        backgroundColor: `color-mix(in srgb, ${course.color || 'hsl(var(--primary))'} 15%, transparent)`
                      }}
                    >
                    ...
                    <div>
                      <p className="text-sm font-medium">
                        {DAYS[slot.day_of_week - 1]}, Stunde {slot.period}{isDouble ? `-${nextPeriod}` : ''}
                      </p>
                      {slot.room && (
                        <p className="text-[10px] text-muted-foreground">Raum {slot.room}</p>
                      )}
                    </div>
                  </div>
                  
                  <Button
                    variant={hasEva || hasEvaNext ? 'default' : 'outline'}
                    size="sm"
                    ...
                    onClick={() => {
                      toggleEva(slot.day_of_week, slot.period, hasEva);
                      if (isDouble && nextPeriod) {
                        toggleEva(slot.day_of_week, nextPeriod, hasEvaNext);
                      }
                    }}
                  >
                    {hasEva || hasEvaNext ? (
            ...
            <Tabs defaultValue="plan">
              ...
              <TabsContent value="plan" className="mt-3">
                {context === 'timetable' ? (
                  ...
                  renderCourseTimetableSlots()
                ) : (
                  <div className="py-8 text-center">
                    <Clock className="w-8 h-8 mx-auto text-muted-foreground/30 mb-2" strokeWidth={1.5} />
                    <p className="text-sm text-muted-foreground">Kein Kurs ausgewaehlt</p>
                  </div>
                )}
              </TabsContent>
