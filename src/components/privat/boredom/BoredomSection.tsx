...
export function BoredomSection({ onBack }: BoredomSectionProps) {
  ...
  const fetchActivities = async () => {
    ...
    const { data, error } = await supabase.from('boredom_activities').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
    if (error) {
      toast({ title: 'Fehler', description: error.message, variant: 'destructive' });
    } else {
      const activitiesWithCounts = await Promise.all((data || []).map(async activity => {
        const { data: skills } = await supabase.from('activity_skills').select('completed').eq('activity_id', activity.id);
        return {
          ...activity,
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) {
      ...
      toast({ title: 'Aktivitaet geloescht' });
      fetchActivities();
    }
  };
  ...
  if (selectedActivity) {
    return <ActivityDetailView activityId={selectedActivity} onBack={() => { setSelectedActivity(null); fetchActivities(); }} />;
  }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm mt-2">Erstelle dein erstes Projekt</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {activities.map(activity => (
            <ActivityCard key={activity.id} activity={activity} onClick={() => setSelectedActivity(activity.id)} onDelete={() => handleDelete(activity.id)} />
          ))}
        </div>
      )}

      <AddActivityDialog open={showAddDialog} onOpenChange={setShowAddDialog} onSuccess={() => { setShowAddDialog(false); fetchActivities(); }} />
    </div>
  );
}
