...
import { Check, Flame, Plus, Settings, ChevronDown } from 'lucide-react';
        ...
        <Collapsible open={showDone} onOpenChange={setShowDone}>
          ...
          <CollapsibleContent className="space-y-1.5 pt-1.5">
            {doneHabits.map(habit => {
              ...
              const streak = habitStreaks[habit.id] || 0;
              return (
                <div key={habit.id} onClick={() => toggle(habit)} className="flex items-center gap-2.5 p-2 rounded-xl cursor-pointer transition-all bg-success/5 opacity-60 hover:opacity-80">
                  <Checkbox checked={true} className="pointer-events-none" />
                  <span className="flex-1 text-sm truncate line-through text-muted-foreground">{habit.name}</span>
                  {streak > 0 && (
                    <div className="flex items-center gap-0.5 shrink-0">
                      <Flame className={`w-3 h-3 ${streak >= 7 ? 'text-orange-500' : 'text-muted-foreground'}`} />
                      <span className={`text-[10px] font-mono ${streak >= 7 ? 'text-orange-500 font-semibold' : 'text-muted-foreground'}`}>{streak}</span>
