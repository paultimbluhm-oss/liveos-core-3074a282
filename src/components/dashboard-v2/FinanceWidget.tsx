...
import { Wallet, TrendingUp, Plus, ArrowUpRight, ArrowDownRight, ChevronDown, Pencil, Check, X, HandCoins } from 'lucide-react';
...
export function FinanceWidget({ size, onOpenSheet }: { size: WidgetSize; onOpenSheet?: () => void }) {
  ...
  const handleLoanAdd = async () => {
    if (!user || !loanPerson || !loanAmount || !loanAccountId) return;
    setLoanSaving(true);
    ...
    await supabase.from('v2_loans').insert({
      user_id: user.id,
      loan_type: loanType,
  ...
  if (size === 'small') {
    return (
      <div className="rounded-2xl bg-card/60 backdrop-blur-xl border border-border/30 p-4 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent/5 pointer-events-none" />
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-7 h-7 rounded-xl bg-primary/10 backdrop-blur-sm flex items-center justify-center">
  ...
  // === MEDIUM / LARGE ===
  return (
    <div className="rounded-2xl bg-card/60 backdrop-blur-xl border border-border/30 p-4 space-y-3 relative overflow-hidden">
      ...
      <div className="relative z-10 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setShowLoanForm(!showLoanForm)}>
              <HandCoins className="w-4 h-4" strokeWidth={1.5} />
            </Button>
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setShowAddTx(!showAddTx)}>
        ...
        {/* Net Worth */}
        <div className="rounded-xl bg-gradient-to-r from-primary/10 via-primary/5 to-transparent backdrop-blur-sm p-3 -mx-1">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl bg-card/40 backdrop-blur-sm border border-border/20 p-2.5">
          ...
          <div className="pt-2 border-t border-border/20 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              ...
              <Select value={loanAccountId || (accounts[0]?.id ?? '')} onValueChange={setLoanAccountId}>
                <SelectTrigger className="h-9 text-xs bg-muted/30 border-border/30">
